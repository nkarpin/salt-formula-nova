...
def _cmd_raising_helper(command_string, test=False):
    if test:
...
def cell_present(name, db_name, db_user, db_password, db_address,
                 ...
                 messaging_engine='rabbitmq', messaging_query_params=None,
                 db_query_params=None, test=False):
    ...
    :param db_query_params: dictionary of query params to append to database
        connection URL. charset=utf8 will always be ensured in query params.
    ...
    if len(cell_info) >= 1:
        ...
        if cell_info['existing_db_connection'] != db_connection:
            command_string = (
                '%s --database_connection \'%%(db_connection)s\'' %
                command_string)
        ...
        try:
            _cmd_raising_helper(
                ('nova-manage cell_v2 update_cell --cell_uuid %s %s' % (
                    cell_info['cell_uuid'], command_string)) % changes,
                test=test)
...
def cell_absent(name, force=False, test=False):
    ...
    try:
        ...
        _cmd_raising_helper(
            'nova-manage cell_v2 delete_cell --cell_uuid %s %s' % (
                cell_uuid, '--force' if force else ''), test=test)
...
def instances_mapped_to_cell(name, timeout=60):
